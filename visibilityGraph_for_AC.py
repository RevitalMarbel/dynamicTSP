...
def CreateVGFromFile(node_f_name, names_f_node, disances_f_name, num_of_sats=600 ,max_neighbors=3, time=0, minDist=1500 , initPher=1, ratio=1):
    file1 = open(node_f_name, 'rb')
    names = pickle.load(file1)
    file1.close()

    file2 = open(names_f_node, 'rb')
    nodes = pickle.load(file2)
    file2.close()
    gList=[]
    ...
    file_out = open(node_f_name+"_graphs", 'ab')
    pickle.dump(gList, file_out)
    file_out.close()
    print("file created",node_f_name+"_graphs")
